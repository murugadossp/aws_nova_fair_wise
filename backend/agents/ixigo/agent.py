...
class IxigoAgent:
    ...
    def search(
        ...
        class_code = self._get_class_code(travel_class)

        url = (
            f"{base_url}/search/result/flight"
            f"?from={from_code}&to={to_code}&date={date_compact}"
            f"&adults=1&children=0&infants=0&class={class_code}&source=Search"
        )
        ...
        try:
            ...
            with Workflow(workflow_definition_name=workflow_name, model_id="nova-act-latest") as wf:
                with NovaAct(workflow=wf, starting_page=url, headless=headless, tty=False) as nova:
                    log.debug("Nova Act browser started for ixigo.com (workflow=%s)", workflow_name)
