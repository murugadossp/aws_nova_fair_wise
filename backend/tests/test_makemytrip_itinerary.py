"""
Test: MakeMyTrip — Single Itinerary Flow

Starts from one harvested MakeMyTrip itinerary / booking URL instead of rerunning
search + harvest. This isolates booking-page fare extraction, coupon extraction,
and optional payment probing.
"""
...
_DEFAULT_LOG_PATH = Path(__file__).resolve().parent.parent / "logs" / "makemytrip_log.txt"
_HARVEST_LINE_RE = re.compile(
    r"Harvest \[\d+/\d+\]: (?P<airline>.+?) (?P<flight_number>[A-Z0-9-]+) → (?P<url>https://www\.makemytrip\.com/[^\s\"']+)"
)
_URL_RE = re.compile(r"https://www\.makemytrip\.com/[^\s\"']+")
...
def _load_latest_harvest_info(log_path: Path) -> dict:
    ...
    raise AssertionError(f"No harvested MakeMyTrip itinerary URL found in {log_path}")
...
def _extract_single_itinerary_offer(itinerary_url: str, airline: str, flight_number: str) -> dict:
    ...
    collect_convenience_fee = bool(
        int(os.environ.get("MMT_ENABLE_PAYMENT_PROBE", "1" if _CONFIG.get("collect_convenience_fee", False) else "0"))
    )
    headless = os.environ.get("FAREWISE_HEADED", "0") != "1"
    return MakeMyTripAgent._extract_offers_from_itinerary_url(
        workflow_name,
        itinerary_url,
        {
            "airline": airline,
...
def test_mmt_single_itinerary(itinerary_url: str | None = None) -> dict:
    log_path = Path(os.environ.get("MMT_LOG_PATH") or _DEFAULT_LOG_PATH)
    if itinerary_url:
        harvest_info = {
            "itinerary_url": itinerary_url,
            "airline": os.environ.get("MMT_ITINERARY_AIRLINE", "UNKNOWN"),
            "flight_number": os.environ.get("MMT_ITINERARY_FLIGHT_NUMBER", "UNKNOWN"),
        }
    else:
        harvest_info = _load_latest_harvest_info(log_path)

    log.info(
        "=== test_mmt_single_itinerary: %s %s ===",
...
if __name__ == "__main__":
    log_path = add_agent_test_file_handler("makemytrip_itinerary")
    log.info("Test logs also written to %s", log_path)
    itinerary_url = os.environ.get("MMT_ITINERARY_URL")
    result = test_mmt_single_itinerary(itinerary_url=itinerary_url)
    log.info("Single itinerary JSON:\n%s", json.dumps(result, indent=2, default=str))
    log.info("MakeMyTrip single-itinerary test DONE")
